...
def get_transformations_pdbx(file_pdbx):
    ...
    return pdbx.convert._get_transformations(
        file_pdbx.get_category('pdbx_struct_oper_list')
    )
...
def get_transformations_mmtf(all_assemblies, world_scale = 0.01):
    ...
    return np.array([item['matrix'] for item in assembly]).reshape(
        (len(assembly), 4, 4)
    )
